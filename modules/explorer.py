...
def query_db(Q, head=5, mk_fl=False):
    ...
    if mk_fl:
        ...
        rs.to_csv('data/output/out.csv', index=False) #, compression=compression_opts)
...
########################################################################################################################
# Get O-Line and TE Play activity for one play
########################################################################################################################
_query = ''' 
        SELECT *
            FROM linemen as L
